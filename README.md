...
RAMMER is a pretrained reaction-aware multimodal AI model that integrates protein sequence, structure, and catalytic reaction information to improve enzyme function annotation, including strong recall at fine-grained EC levels.

At large scale, RAMMER has been applied to billions of proteins across diverse environments and supports function-driven mining of valuable enzymes by linking sequence/structure signals with catalytic behavior. This repository provides practical scripts and workflows for:

- Zero-shot EC prediction (`top1` and `max-separation` strategies)
- Binary enzyme/non-enzyme classification on top of RAMMER embeddings
- Distributed training reproduction (DDP with `torchrun`)

Project assets (`Background_library`, `data`, and `model`) are available on Hugging Face: [PengJiaMa123/RAMMER](https://huggingface.co/PengJiaMa123/RAMMER).

## Repository Structure

- `set1_get_RAMMER_embedding.py`: generate RAMMER embeddings from FASTA/FAA input
...
- `binary_enzyme_classifier.py`: enzyme/non-enzyme binary classifier from RAMMER embeddings
...
```bash
conda create -n rammer python=3.10 -y
conda activate rammer
...
Download project resources from [PengJiaMa123/RAMMER](https://huggingface.co/PengJiaMa123/RAMMER) and place them under the project root:
...
```bash
python ./set1_get_RAMMER_embedding.py --input_name NEW-392
```
...
Prediction CSVs for RAMMER and other methods on `new392` / `ram255` are located in:

- `./data/test_data`

## Enzyme / Non-enzyme Classification

This task requires RAMMER embedding `.h5` files under `./RAMMER_embedding`.
